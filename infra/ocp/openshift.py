#!/usr/bin/env python3
"""Deploy NCS Advisor to an existing OpenShift project (e.g. a Developer
Sandbox / trial cluster) without shell wrappers.

Three deployables only: postgres (pgvector), ollama (in-cluster embeddings),
and ui (Streamlit). The LangGraph runs in-process inside the ui — there is no
...
for you. It targets an existing project, builds the ui image with an OpenShift
BuildConfig (binary Docker-strategy build pushed straight from your checkout
into the project's internal registry via an ImageStream), and applies the
Deployment/StatefulSets/Route.
"""
...
def render_and_apply(config: Config, tag: str = "latest",
    ...
    with tempfile.TemporaryDirectory(prefix="ncs-ocp-") as temp_dir:
...
def sync_secret(config: Config) -> None:
    ...
    # Non-secret values (Vertex project/location/model, Ollama, PG host/port)
    # live in the ncs-config ConfigMap. The Secret keeps only the Postgres
    # credentials (and any API key such as NVD_API_KEY).
    excluded_prefixes = ("OLLAMA_", "INGEST_", "NCS_", "ANTHROPIC_", "PGHOST", "PGPORT")
    filtered = []
    ...
    with tempfile.NamedTemporaryFile(mode="w", encoding="utf-8") as secret_file:
        ...
        secret = command([
            ...
            "ncs-secrets", f"--from-env-file={secret_file.name}",
...
def sync_google_credentials(config: Config) -> None:
    ...
    (OpenShift has no Workload Identity). If .application_default_credentials.json
    is absent, this is skipped — the ui Deployment mounts the secret as
    `optional`, so the pod still starts (Vertex calls will fail until it is
    provided)."""
...
def create_secrets_and_schema(config: Config) -> None:
    ...
    schema = command([
        "oc", "-n", config.namespace, "create", "configmap", "ncs-postgres-init",
...
def bootstrap(config: Config) -> None:
    """One-time setup: service account + image stream/build config.
    Safe to re-run."""
    check_login()
...
def deploy(config: Config, tag: str) -> None:
    ...
    oc(config, ["rollout", "status", "deployment/ui", "--timeout=180s"])
...
def sync_deployment_secrets(config: Config) -> None:
    check_login()
    create_secrets_and_schema(config)
    oc(config, ["rollout", "restart", "deployment/ui"])
    oc(config, ["rollout", "status", "deployment/ui", "--timeout=180s"])
...
def main() -> None:
    ...
    subcommands.add_parser("bootstrap", help="Service account, image stream/build config")
    deploy_parser = subcommands.add_parser("deploy", help="Build image and deploy NCS Advisor")
    deploy_parser.add_argument("--tag", default="latest")
    subcommands.add_parser("sync-secrets", help="Update secrets/configmaps from .env")
    subcommands.add_parser("routes", help="Print the NCS Advisor route URLs")
    subcommands.add_parser("grant-anyuid", help="Troubleshooting only: grant anyuid SCC (needs project admin)")
    teardown_parser = subcommands.add_parser("teardown", help="Delete all NCS Advisor resources in the project")
