#!/usr/bin/env python3
"""Copy the local NCS Advisor pgvector database to/from Postgres running on
...
def namespace_default() -> str:
    ...
    if ns:
        return ns
    return run(["oc", "project", "-q"], capture=True).strip() or "ncs"


def replicas(namespace: str) -> str:
    return run([
        "oc", "-n", namespace, "get", "deployment/ui",
...
def restore(source: Path, namespace: str) -> None:
    ...
    run(["oc", "-n", namespace, "scale", "deployment/ui", "--replicas=0"])
    try:
        ...
        run([
            "oc", "-n", namespace, "scale", "deployment/ui",
