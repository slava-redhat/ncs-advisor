#!/usr/bin/env python3
"""Provision and operate the NCS Advisor GKE deployment without shell wrappers.
...
class Config:
    ...
    def from_environment(cls) -> "Config":
        ...
        return cls(
            ...
            cluster=os.environ.get("GKE_CLUSTER_NAME", "ncs"),
            repository=os.environ.get("ARTIFACT_REPOSITORY", "ncs"),
            namespace=os.environ.get("K8S_NAMESPACE", "ncs"),
            tier=tier,
            # Google SA bound to the ncs KSA via Workload Identity (Vertex).
            gsa_email=os.environ.get(
                "GSA_EMAIL", f"ncs-vertex@{project}.iam.gserviceaccount.com"),
        )
...
def provision(config: Config) -> None:
    ...
    if not exists(["gcloud", "artifacts", "repositories", "describe", config.repository,
        ...
        command([
            ...
            "--description", "NCS Advisor container images", "--project", config.project,
        ])
    ...
    if not exists(["gcloud", "container", "clusters", "describe", config.cluster,
        ...
        command([
            ...
            # Workload Identity so the ui can reach Claude-on-Vertex without an
            # ADC key file. The KSA<->GSA IAM binding is a one-time manual step
            # (see GKE-WORKFLOW.md).
            "--workload-pool", f"{config.project}.svc.id.goog",
...
def render_and_apply(config: Config, tag: str = "latest",
    ...
    with tempfile.TemporaryDirectory(prefix="ncs-k8s-") as temp_dir:
...
def sync_secret(config: Config) -> None:
    ...
    # Everything non-secret (Vertex project/location/model, Ollama, PG host/port)
    # lives in the ncs-config ConfigMap instead. The Secret keeps only the
    # Postgres credentials (and any API key such as NVD_API_KEY).
...
def print_ingress_address(config: Config, wait: bool = False) -> None:
    ...
    while True:
        ...
        if address:
            print(f"NCS Advisor is available at http://{address}/")
            return
...
def main() -> None:
    ...
    ingress_parser = subcommands.add_parser("ingress", help="Print the NCS Advisor ingress URL")
    ingress_parser.add_argument("--wait", action="store_true",
                                help="Wait up to five minutes for an ingress address")
    teardown_parser = subcommands.add_parser("teardown", help="Delete all NCS Advisor GCP resources")
