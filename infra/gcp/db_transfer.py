#!/usr/bin/env python3
"""Copy the local NCS Advisor pgvector database to a provisioned Postgres pod."""
...
def backup(output: Path) -> None:
    ...
    dump = subprocess.Popen(
        [
            ...
            "--clean", "--if-exists", "-U", os.environ.get("POSTGRES_USER", "ncs"),
            "-d", os.environ.get("POSTGRES_DB", "ncs"),
        ],
        cwd=ROOT,
...
def replicas(namespace: str) -> str:
    return run([
        "kubectl", "-n", namespace, "get", "deployment/ui",
...
def restore(source: Path, namespace: str) -> None:
    ...
    run(["kubectl", "-n", namespace, "scale", "deployment/ui", "--replicas=0"])
    try:
        restore_process = subprocess.Popen(
            [
                ...
                "-h", "127.0.0.1",
                "-U", os.environ.get("POSTGRES_USER", "ncs"),
                "-d", os.environ.get("POSTGRES_DB", "ncs"),
            ],
        ...
        run([
            "kubectl", "-n", namespace, "scale", "deployment/ui",
...
def main() -> None:
    ...
    restore_parser.add_argument("--namespace", default=os.environ.get("K8S_NAMESPACE", "ncs"))
